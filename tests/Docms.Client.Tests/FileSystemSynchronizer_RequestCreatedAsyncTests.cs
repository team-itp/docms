...
{
    [TestClass]
    public class FileSystemSynchronizer_RequestCreatedAsyncTests
    {
        ...
        {
            await sut.RequestCreatedAsync("test/test1.txt");
        ...
        {
            await mockClient.CreateOrUpdateDocumentAsync("test/test1.txt", CreateStream("Hello"));
            await sut.RequestCreatedAsync("test/test1.txt");
        ...
        public async Task ファイルが存在する場合でファイルがサーバーの最新と一致しない場合ファイルをアップロードする()
        {
            ...
            await localFileStorage.Create("test/test1.txt", CreateStream("Hello new"), now, now);
            await sut.RequestCreatedAsync("test/test1.txt");
